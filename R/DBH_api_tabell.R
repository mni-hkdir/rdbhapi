    ...
    for (i in seq_along(filters)) {
      ...
      if (filters[[i]][1] == "*") {
        ...
        filter_query[[i]]$selection$values <-
          lapply(filters[[i]][2], function(s) enc2utf8(as.character(s)))
        if (names(filters)[i] %in% names(exclude)) {
          filter_query[[i]]$selection$exclude <-
            lapply(exclude[[names(filters)[i]]], function(s) enc2utf8(as.character(s)))
        }
      } else if (filters[[i]][1] == "between") {
        filter_query[[i]]$selection$filter <- "between"
        filter_query[[i]]$selection$values <-
          c(lapply(filters[[i]][2], function(s) enc2utf8(as.character(s))),
